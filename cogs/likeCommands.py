...
class LikeCommands(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.api_host = "http://like.thug4ff.xyz/like?uid={uid}&server_name={region}&key=abhi45"
        ...
        if RAPIDAPI_KEY:
            self.headers = {
                ...
                'x-rapidapi-host': "http://like.thug4ff.xyz/like?uid={uid}&server_name={region}&key=abhi45"
            }
        ...
        try:
            async with ctx.typing():
                async with self.session.get(f"{self.api_host}/like?uid={uid}", headers=self.headers) as response:
                    ...
                    embed.set_footer(text="DEVELOPED BY ABHINANDAN")
                    embed.description += "\n🔗 JOIN : hhttps://discord.gg/GmuctQ8m"
